...
```{r}
if(!is.null(df$output$validation_metrics)){
gauge(round(df$output$validation_metrics$AUC,2), min = 0.5, max=1)
}
```
...
```{r}
if(!is.null(df$output$validation_metrics)){
metricas_validation[which.max(metricas_validation$f1), c("f1","threshold")]
}
```
...
if(!is.null(df$output$validation_metrics)){
v_gain_lift_data <- as.data.frame(t(df$output$validation_metrics$gains_lift_table$data))
colnames(v_gain_lift_data) <- df$output$validation_metrics$gains_lift_table$columns$name

v_gain_lift_data[,] <- sapply(v_gain_lift_data[,], function(x) as.numeric(as.character(x)))

DT::datatable(round(v_gain_lift_data,2), rownames = FALSE)
}
```
...
```{r}
if(!is.null(df$output$validation_metrics)){
ggplot(v_gain_lift_data, aes(x=cumulative_data_fraction, y = lift)) +
  geom_line(color="darkorange")
}
```
